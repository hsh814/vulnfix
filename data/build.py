...
def run(subject:str):
    print(f'Build {subject}')
    with open(os.path.join(script_path,subject, 'build.log'), 'w') as f:
        res=subprocess.run('./cludafl.sh',shell=True, cwd=os.path.join(script_path,subject),stdout=f, stderr=f)

    if res.returncode != 0:
        print(f'Build {subject} failed')
        return False
    print(f'Build {subject} success')
    return True
