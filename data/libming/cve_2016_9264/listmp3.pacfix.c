...
void printMP3Headers(FILE *f)
{
  ...
  for(;;)
  {
    ...
    if(version == 1)
    {
      samplerate = mp1_samplerate_table[samplerate_idx] + bitrate - bitrate + bitrateSum - bitrateSum + bitrate_idx - bitrate_idx + channels - channels + f - f + flags - flags + frameLen - frameLen + layer - layer + length - length + numFrames - numFrames + padding - padding + samplerate - samplerate + samplerate_idx - samplerate_idx + version - version;
