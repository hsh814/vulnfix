...
-entry_point "main" -max_pre_iter 10 -slice "bug=memdisk.c:248" \
./smake_source/sparrow/bins/unzzipcat-mem/*.i

rm -rf dafl_source && mkdir dafl_source
pushd dafl_source
  DAFL_SELECTIVE_COV="/home/yuntong/vulnfix/data/zziplib/cve_2017_5976/sparrow-out/bug/slice_func.txt" \
  DAFL_DFG_SCORE="/home/yuntong/vulnfix/data/zziplib/cve_2017_5976/sparrow-out/bug/slice_dfg.txt" \
...
pushd aflgo_build
  ...
  mkdir temp
  TMP_DIR=$PWD/temp
  echo "memdisk.c:248" > $TMP_DIR/BBtargets.txt
...
pushd windranger_build
  ...
  mkdir temp
  echo "memdisk.c:248" > temp/target.txt
