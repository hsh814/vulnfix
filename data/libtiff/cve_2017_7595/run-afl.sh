...
TARGET_DIR="$VULNFIX_HOME/vulnfix/data/libtiff/cve_2017_7595"
...
for file in "$SEED_DIR"*; do
    ...
    export AFL_NO_UI=1
    export PACFIX_TARGET_LINE=1687
    ...
    $AFL_CMD $AFL_OPTS -i "$input_dir" -p "$AFL_PROG" -o "$output_dir" -b "$EXPLOIT" -- "$INSTRUMENTED_PROG" -i @@ ${OUTPUT_TMP} &
