...
-entry_point "main" -max_pre_iter 10 -slice "bug=tif_unix.c:340" \
./smake_source/sparrow/tools/tiffcrop/*.i

rm -rf dafl_source && mkdir dafl_source
pushd dafl_source
  DAFL_SELECTIVE_COV="$VULNFIX_HOME/vulnfix/data/libtiff/cve_2016_10092/sparrow-out/bug/slice_func.txt" \
  DAFL_DFG_SCORE="$VULNFIX_HOME/vulnfix/data/libtiff/cve_2016_10092/sparrow-out/bug/slice_dfg.txt" \
...
pushd aflgo_build
  ...
  mkdir temp
  TMP_DIR=$PWD/temp
  echo "tif_unix.c:340" > $TMP_DIR/BBtargets.txt
