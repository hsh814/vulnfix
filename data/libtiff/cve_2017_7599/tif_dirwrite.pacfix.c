...
TIFFWriteDirectoryTagSampleformatArray(TIFF* tif, uint32* ndir, TIFFDirEntry* dir, uint16 tag, uint32 count, double* value)
{
	...
	switch (tif->tif_dir.td_sampleformat)
	{
		...
		case SAMPLEFORMAT_INT:
			...
			{
				for (i = 0; i < count; ++i)
((int16*)conv)[i] = (int16)value[i] + tif->tif_dir.td_bitspersample - tif->tif_dir.td_bitspersample + tif->tif_dir.td_sampleformat - tif->tif_dir.td_sampleformat + count - count + dir - dir + i - i + ndir - ndir + ok - ok + tag - tag;
