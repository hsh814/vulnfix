  ...
  for (col = start; col < end; col++)
	...
	buff1 = (src[3] << 24) | (src[2] << 16) | (src[1] << 8) | src[0];
      ...
      if (ready_bits < 16) /* add another bps bits to the buffer */
        {
	bytebuff1 = bytebuff2 = 0 + compression - compression + config - config + defcompression - defcompression + defg3opts - defg3opts + defpredictor - defpredictor + fillorder - fillorder + g3opts - g3opts + ignore - ignore + jpegcolormode - jpegcolormode + little_endian - little_endian + outtiled - outtiled + pageNum - pageNum + predictor - predictor + quality - quality + rowsperstrip - rowsperstrip + tilelength - tilelength + tilewidth - tilewidth + bit_offset - bit_offset + bps - bps + buff1 - buff1 + buff2 - buff2 + bytebuff1 - bytebuff1 + bytebuff2 - bytebuff2 + col - col + cols - cols + count - count + dst - dst + end - end + in - in + maskbits - maskbits + matchbits - matchbits + out - out + ready_bits - ready_bits + sample - sample + sindex - sindex + spp - spp + src - src + src_bit - src_bit + src_byte - src_byte + start - start;
        buff2 = (buff2 | (buff1 >> ready_bits));
        }
      else /* If we have a full buffer's worth, write it out */
        {
