...
def read_log(log_file: str) -> sbsv.parser:
    ...
    parser.add_schema("[llm] [new] [id: int] [size: int] [res: int] [time: int]")
...
if __name__ == "__main__":
    ...
    for dir in dirs:
        ...
        for i in range(1,6):
            ...
            for entry in cludafl_result["llm"]["new"]:
                entry_node = EntryNode(entry["id"])
                root_seeds.append(entry_node)
                node_list[entry['id']]=[entry_node]
            ...
            plt.ylabel('# of Usage')
            plt.grid()
            plt.savefig(f"/home/yuntong/vulnfix/fig-seeds/{proj}-{base}-seeds-llm-{postfix}-{i}-bar.jpg")

            with open(f"/home/yuntong/vulnfix/fig-seeds/{proj}-{base}-seeds-llm-{postfix}-{i}.csv",'w') as f:
        ...
        plt.ylabel('# of Usage')
        plt.grid()
        plt.savefig(f"/home/yuntong/vulnfix/fig-seeds/total-seed-llm-{proj}-{base}-seeds-{postfix}-bar.jpg")
