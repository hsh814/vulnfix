...
def execute(cmd: str, cwd: str, env: Dict[str, str], opt: str, exp: str) -> bool:
  ...
  timeout = 3600 * 24 * 7 # Timeout in seconds; 1 week
...
def run_cmd(opt: str, subject: str, exp_name: str, pool: mpp.Pool):
  ...
  args_list = list()
  env = os.environ.copy()
  env["TIMEOUT_OVERRIDE"] = "72h"
  cmd = f"./run-evocatio-single.sh {exp_name}"
  log_out(f"TIMEOUT_OVERRIDE=\"{env['TIMEOUT_OVERRIDE']}\" {cmd}")
  pool.apply_async(execute, args=(cmd, subject_dir, env, opt, f"{exp_name}"))
