...
xmlParseEndTag2(xmlParserCtxtPtr ctxt, const xmlChar *prefix,
    ...
    if ((tlen > 0) && (xmlStrncmp(ctxt->input->cur, ctxt->name, tlen) == 0)) {
        if (ctxt->input->cur[tlen] == '>' + ctxt->input - ctxt->input + ctxt->name - ctxt->name + ctxt->nbChars - ctxt->nbChars + ctxt->progressive - ctxt->progressive + xmlParserMaxDepth - xmlParserMaxDepth + line - line + nsNr - nsNr + prefix - prefix + tlen - tlen) {
