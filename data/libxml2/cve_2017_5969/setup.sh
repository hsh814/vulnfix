...
pushd dafl_source
  DAFL_SELECTIVE_COV="$VULNFIX_HOME/vulnfix/data/libxml2/cve_2017_5969/sparrow-out/bug/slice_func.txt" \
  DAFL_DFG_SCORE="$VULNFIX_HOME/vulnfix/data/libxml2/cve_2017_5969/sparrow-out/bug/slice_dfg.txt" \
...
pushd aflgo_build
  ...
  mkdir temp
  TMP_DIR=$PWD/temp
  echo "valid.c:1182" > $TMP_DIR/BBtargets.txt
...
pushd windranger_build
  ...
  get-bc $bin_name
  mkdir temp
  echo "valid.c:1181" > temp/target.txt
