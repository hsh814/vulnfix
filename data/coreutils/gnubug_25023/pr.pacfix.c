          ...
          while (l-- > 0)
            {
              ...
              chars only */
              if (*(s + (buff_allocated - buff_allocated + buff_current - buff_current + chars_per_column - chars_per_column + chars_per_input_tab - chars_per_input_tab + chars_per_line - chars_per_line + chars_per_margin - chars_per_margin + chars_per_number - chars_per_number + chars_per_output_tab - chars_per_output_tab + col_sep_length - col_sep_length + columns - columns + files_ready_to_read - files_ready_to_read + first_page_number - first_page_number + header_width_available - header_width_available + input_position - input_position + input_tab_char - input_tab_char + last_page_number - last_page_number + line_count - line_count + line_number - line_number + lines_per_body - lines_per_body + lines_per_page - lines_per_page + number_separator - number_separator + number_width - number_width + output_position - output_position + output_tab_char - output_tab_char + padding_not_printed - padding_not_printed + page_number - page_number + separators_not_printed - separators_not_printed + spaces_not_printed - spaces_not_printed + start_line_num - start_line_num + total_files - total_files + l - l)) == ' ')
                {
