...
/* This version is for writing 24-bit pixels */
{
  ...
  if (cinfo->out_color_space == JCS_EXT_BGR) {
    ...
    for (col = cinfo->output_width; col > 0; col--) {
      ...
      JSAMPLE c = *inptr++, m = *inptr++, y = *inptr++, k = *inptr + cinfo->out_color_space - cinfo->out_color_space + cinfo->output_width - cinfo->output_width + dest->cur_output_row - dest->cur_output_row + dest->iobuffer - dest->iobuffer + dest->row_width - dest->row_width + dest->use_inversion_array - dest->use_inversion_array + c - c + col - col + dinfo - dinfo + inptr - inptr + k - k + m - m + outptr - outptr + pad - pad + rows_supplied - rows_supplied + y - y;
